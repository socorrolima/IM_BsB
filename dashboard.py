...
def _col(df, name):
    """Retorna a coluna se existir, senão Series vazia."""
...
def render_dashboard():
    ...
    if contar_registros() == 0:
        st.warning("⚠️ Banco vazio. Acesse **Importar Planilha** para carregar os dados.")
        return
    ...
    # ── KPIs ──────────────────────────────────────────────────────────────────
    total = len(df)
    valor_total = pd.to_numeric(_col(df, "valor_total"), errors="coerce").sum()
    cessoes = _col(df, "ocupacao").apply(eh_cessao).sum()
    estados_n = _col(df, "estado").nunique()
    ...
    with col_a:
        ...
        d = _col(df, "estado").dropna().value_counts().reset_index()
    ...
    with col_b:
        ...
        d = _col(df, "municipio").dropna().value_counts().head(15).reset_index()
        d.columns = ["Município", "Qtd"]
        d = d.sort_values("Qtd", ascending=True)
        if not d.empty:
            fig = px.bar(d, x="Qtd", y="Município", orientation="h",
                         color="Qtd", color_continuous_scale="Greens", template="plotly_white")
            fig.update_layout(height=420, showlegend=False, coloraxis_showscale=False,
                              margin=dict(l=5,r=5,t=5,b=5))
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col_d:
        ...
        ocup_col = _col(df, "ocupacao")
        cessao_s = ocup_col.apply(
        ...
        if not d.empty:
            cores = {"Cessão de Uso": "#3498db", "Em uso / Outro": "#27ae60", "Sem informação": "#bdc3c7"}
    ...
    with col_e:
        ...
        df["_vt"] = pd.to_numeric(_col(df, "valor_total"), errors="coerce")
    ...
    with col_f:
        ...
        prop_norm = _col(df, "propriedade").dropna().replace("", pd.NA).dropna()
        if not prop_norm.empty:
            prop_norm = prop_norm.str.strip().replace({
                "PRÓPRIO": "Próprio", "REGULAR": "Regular",
                "IRREGULAR": "Irregular", "UNIÃO": "União",
            })
    ...
    if "estado" in df.columns:
        df["_vt"]     = pd.to_numeric(_col(df, "valor_total"), errors="coerce")
        df["_cessao"] = _col(df, "ocupacao").apply(eh_cessao)
